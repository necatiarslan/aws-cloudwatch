...
class TreeDataProvider {
    ...
    GetVisibleTreeItemList() {
        ...
        for (var node of this.treeItemList) {
            if (treeView_1.TreeView.Current && treeView_1.TreeView.Current.FilterString && !node.doesFilterMatch(treeView_1.TreeView.Current.FilterString)) {
                continue;
            }
            if (treeView_1.TreeView.Current && treeView_1.TreeView.Current.isShowOnlyFavorite && !node.IsFav) {
                continue;
            }
            result.push(node);
        }
